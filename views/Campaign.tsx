...
export const Campaign: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (sessionId && user && !paymentCompleted) {
            // Payment was successful, record it and proceed
            ...
            recordPaymentSuccess(user.id, sessionId, PAYMENT_AMOUNT, 'USD')
                .then(() => {
                    ...
                    alert('¡Pago exitoso! Ahora puedes procesar el análisis.');
                })
                .catch(err => {
    ...
    const handleStripeCheckout = async () => {
        ...
        try {
            const checkoutUrl = await createCheckoutSession(user.id, campaign.id);
            // Redirect to Stripe Checkout
            window.location.href = checkoutUrl;
        } catch (error: any) {
            console.error("Stripe checkout failed:", error);
            setPaymentError(error.message || "Error al iniciar el pago. Intenta de nuevo.");
            ...
            {showPaymentModal && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-6">
                    <div className="glass-panel p-6 rounded-2xl w-full max-w-[340px] text-center relative animate-in zoom-in-95">
                        ...
                        >
                            {paymentProcessing ? 'Redirigiendo a pago...' : `Pagar US$${PAYMENT_AMOUNT.toFixed(2)}`}
                        </button>
                        <p className="text-[10px] text-gray-400 mt-2">Pago seguro procesado por Stripe</p>
                    </div>
                </div>
            )}
        </div>
